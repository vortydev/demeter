...
exports.findAllCategoryRecipe = (req, res) => {
  ...
  var condition = categoryrecipe
    ? { category: { [Op.like]: `%${categoryrecipe}%` } }
    : null;
  ...
  CategoryRecipe.findAll({ where: condition })
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message || "Some error occurred while retrieving Mesurement.",
      });
    });
};
  ...
  Rel_ProductRecipe.create(rpr)
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Some error occurred while creating the Rel_ProductRecipe.",
      });
    });
};
  ...
  Rel_ProductRecipe.findAll({ where: conditionR })
    ...
    .catch((err) => {
      res.status(500).send({
        message:
          err.message ||
          "Some error occurred while retrieving Rel_ProductRecipe.",
      });
    });
};
...
exports.updateRPR = (req, res) => {
  ...
  var conditionP = product
    ? { productId: { [Op.like]: `%${product}%` } }
    : null;
...
exports.deleteRPR = (req, res) => {
  console.log("in delete RPR");
  const recipe = req.query.recipeId ? req.query.recipeId : null;
  const product = req.query.productId ? req.query.productId : null;

  if (recipe && product) {
    Rel_ProductRecipe.destroy({
      where: { recipeId: recipe, productId: product },
    })
      .then((num) => {
        if (num == 1) {
          res.send({
          ...
          res.send({
            message: `Cannot delete Recipe with recetteId=${recipe} and productId=${recipe} . Maybe Rel_ProductRecipe was not found!`,
          });
        }
      })
      .catch((err) => {
        res.status(500).send({
          message:
            "Could not delete Rel_ProductRecipe with recipeId=" +
            recipe +
            "or productId=" +
            product,
        });
      });
  } else if(recipe && product === null){
    Rel_ProductRecipe.destroy({
